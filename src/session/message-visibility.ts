...
export function toVisibleHistoryMessage(message: HistoryMessageLike): HistoryMessageLike | null {
    if (message.role === 'tool') {
        return null;
    }

    if (message.role === 'user' && isSetupOrInternalUserMessage(message.content)) {
        return null;
    }

    if (message.role === 'assistant' && isHiddenAssistantContent(message.content)) {
